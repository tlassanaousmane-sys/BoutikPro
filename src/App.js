    ...
    <div style={{ display: 'flex', justifyContent: 'center', padding: 40 }}>
      <div
        style={{
          width: 32,
          height: 32,
          border: `3px solid ${G.brandLight}`,
          borderTop: `3px solid ${G.brand}`,
          borderRadius: '50%',
...
function printReceipt(
  ...
  win.document.write(`<html><head><title>Reçu #${num}</title><style>
    ...
    <div class="sub">Reçu N° ${num} — ${new Date().toLocaleDateString(
    'fr-FR'
    ...
    ${items
      .map(
        (it) =>
          `<div class="row"><span>${it.nom} x${it.qty}</span><span>${Math.round(
            it.prix * it.qty
          ).toLocaleString()} F</span></div>`
      )
      .join('')}
    <hr>
    <div class="row bold"><span>Total</span><span>${Math.round(
      total
    ).toLocaleString()} F</span></div>
    ${
      discount > 0
        ? `<div class="row" style="color:#854F0B"><span>Remise</span><span>-${Math.round(
            discount
          ).toLocaleString()} F</span></div>`
        : ''
    }
    <div class="row" style="color:#0F6E56"><span>Payé (${payMode})</span><span>${Math.round(
    received || net
  ).toLocaleString()} F</span></div>
    ${
      monnaie > 0
        ? `<div class="row"><span>Monnaie rendue</span><span>${Math.round(
            monnaie
          ).toLocaleString()} F</span></div>`
        : ''
    }
    <hr><div class="thanks">Merci pour votre achat !<br>À bientôt chez ${
      boutique.nom
    }</div>
  </body></html>`);
  win.document.close();
    ...
    try {
      if (mode === 'login') {
        const rows = await db.get(
          'boutiques',
          `?email=eq.${encodeURIComponent(form.email)}&select=*`
        );
        ...
        const b = rows[0];
        if (b.password_hash !== btoa(form.password)) {
        ...
        const ex = await db.get(
          'boutiques',
          `?email=eq.${encodeURIComponent(form.email)}`
        );
        if (ex && ex.length > 0) {
        ...
        const rows = await db.post('boutiques', {
          nom: form.nom,
          tel: form.tel,
      ...
      setError('Erreur réseau. Vérifie ta clé Supabase.');
      console.error(e);
    }
    setLoading(false);
  }
        ...
        {mode === 'register' && (
          <>
            <Inp
              ...
              placeholder="ex: Boutique Awa Mode"
            />
            <Inp
              label="Téléphone"
              value={form.tel}
            ...
            <Inp
              ...
              placeholder="Marché Sandaga, Dakar"
            />
          </>
        )}
        <Inp
          ...
          {mode === 'login' ? (
            <>
              ...
              <span
                ...
                style={{ color: G.brand, cursor: 'pointer', fontWeight: 500 }}
              >
                S'inscrire
              </span>
            </>
...
function Dashboard({ boutique, produits, ventes }) {
  ...
  const metrics = [
    ...
    },
    {
      ...
      val: fmt(valeurStock),
      sub: produits.reduce((s, p) => s + p.stock, 0) + ' articles',
      color: '#854F0B',
    },
    {
      label: 'Alertes stock',
      val: stockFaible.length,
      sub: 'produits faibles',
      color: stockFaible.length > 0 ? G.danger : G.brand,
    },
  ];
          ...
          >
            <div>
              <div style={{ fontSize: 13, fontWeight: 500 }}>
                Reçu #{v.receipt_num} {v.client_nom ? `— ${v.client_nom}` : ''}
              </div>
              <div style={{ fontSize: 11, color: '#aaa' }}>
                {fmtDate(v.created_at)} {fmtTime(v.created_at)} ·{' '}
                {v.items.length} article(s) · {v.pay_mode}
              </div>
            </div>
            <span style={{ fontWeight: 700, color: G.brand }}>
              {fmt(v.total)}
            </span>
          </div>
        ))}
...
function Produits({ boutique, produits, setProduits }) {
  ...
  return (
    <div>
              ...
              return (
                <tr key={p.id} style={{ borderBottom: '1px solid #f8f8f8' }}>
                  ...
                  <td
                    style={{
                      padding: 10,
                      color: p.stock <= p.stock_min ? G.danger : '#333',
                      fontWeight: 500,
                    }}
                  >
                    {p.stock}
                  ...
                  <td style={{ padding: 10 }}>
                    <div style={{ display: 'flex', gap: 6 }}>
                      ...
                      <Btn small color="red" onClick={() => del(p.id)}>
                        ✕
                      </Btn>
                    </div>
                  </td>
      ...
      {modal && (
        <Modal
          title={modal === 'add' ? 'Nouveau produit' : 'Modifier produit'}
        ...
        >
          ...
          <Inp
            ...
            placeholder="ex: Tissu, Habillement, Chaussure..."
          />
          <div
          ...
          >
            <Btn color="gray" onClick={() => setModal(null)}>
              Annuler
            </Btn>
...
function Caisse({ boutique, produits, setProduits, ventes, setVentes }) {
  ...
  const dispo = produits.filter(
    (p) => p.stock > 0 && p.nom.toLowerCase().includes(search.toLowerCase())
  );
  ...
  function addToPanier(p) {
    ...
    setPanier(
      ex
        ? panier.map((x) => (x.id === p.id ? { ...x, qty: x.qty + 1 } : x))
        : [...panier, { ...p, qty: 1 }]
    );
  }
  ...
  function changeQty(id, qty) {
    setPanier(
      qty <= 0
        ? panier.filter((x) => x.id !== id)
        : panier.map((x) => (x.id === id ? { ...x, qty } : x))
    );
  }
  ...
  async function valider() {
    ...
    const num =
      (ventes.length > 0
        ? Math.max(...ventes.map((v) => v.receipt_num))
        : 1000) + 1;
    try {
      const rows = await db.post('ventes', {
        boutique_id: boutique.id,
        receipt_num: num,
        total: net,
        discount: +discount || 0,
      ...
      setProduits(
        produits.map((p) => {
          const it = panier.find((x) => x.id === p.id);
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 20 }}>
        ...
        >
          {dispo.map((p) => (
            <div
              ...
              onMouseEnter={(e) =>
                (e.currentTarget.style.borderColor = G.brand)
              }
              onMouseLeave={(e) => (e.currentTarget.style.borderColor = '#eee')}
            >
              <div style={{ fontSize: 24, marginBottom: 4 }}>
                {CAT_ICONS[p.categorie] || '📦'}
              </div>
              <div
                style={{
                  fontSize: 12,
              ...
              <div style={{ fontSize: 13, color: G.brand, fontWeight: 700 }}>
                {fmt(p.prix)}
              </div>
              <div style={{ fontSize: 11, color: '#aaa' }}>
                Stock: {p.stock}
      ...
      >
          ...
          panier.map((it) => (
            ...
            >
              <div style={{ flex: 1 }}>
                ...
                <div style={{ color: G.brand, fontSize: 12 }}>
                  {fmt(it.prix)}
                </div>
              </div>
        ...
        <div
          style={{ borderTop: '1px dashed #ddd', marginTop: 8, paddingTop: 10 }}
        >
          <input
          ...
          >
            ...
            >
              {['Espèces', 'Wave', 'Orange Money', 'Carte'].map((m) => (
                <option key={m}>{m}</option>
              ))}
            </select>
          </div>
          <input
            placeholder="Montant reçu (F)"
            type="number"
            value={received}
          ...
          {+received > 0 && (
            ...
            >
              ...
              <span
                style={{
                  fontWeight: 600,
                  color: monnaie >= 0 ? G.brand : G.danger,
                }}
              >
          ...
          >
            <span>Total</span>
            <span style={{ color: G.brand }}>{fmt(net)}</span>
          </div>
          <Btn full onClick={valider} disabled={panier.length === 0 || saving}>
            {saving ? 'Enregistrement...' : '✓ Valider & Imprimer'}
          </Btn>
        </div>
      </div>
      {done && (
        <Modal title="Vente enregistrée !" onClose={() => setDone(null)}>
          <div style={{ textAlign: 'center', padding: '10px 0' }}>
            ...
            <div style={{ fontSize: 13, color: '#888', marginBottom: 16 }}>
              {fmt(done.total)} · {done.payMode}
            </div>
            <Btn full color="gray" onClick={() => setDone(null)}>
              Nouvelle vente
            </Btn>
          </div>
        </Modal>
    ...
    <div>
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 22, fontWeight: 700, color: G.brand }}>
            {fmt(total)}
          </div>
        </div>
      ...
      {[...ventes].reverse().map((v) => (
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 11, color: '#aaa' }}>
              {fmtDate(v.created_at)} {fmtTime(v.created_at)} · {v.items.length}{' '}
              article(s) · {v.pay_mode}
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontWeight: 700, color: G.brand }}>
              {fmt(v.total)}
            </div>
      ...
      {detail && (
        <Modal
          title={`Vente #${detail.receipt_num}`}
          onClose={() => setDetail(null)}
        >
          <div style={{ fontSize: 13 }}>
            <div style={{ marginBottom: 10, color: '#888' }}>
              {fmtDate(detail.created_at)} à {fmtTime(detail.created_at)} ·{' '}
              {detail.pay_mode}
            ...
            {detail.items.map((it, i) => (
              ...
              >
                ...
                <span style={{ fontWeight: 600 }}>{fmt(it.prix * it.qty)}</span>
              </div>
            ))}
            {detail.discount > 0 && (
              <div
            ...
            >
              <span>Total</span>
              <span style={{ color: G.brand }}>{fmt(detail.total)}</span>
            </div>
              ...
              <Btn
                ...
                onClick={() =>
                  printReceipt(
                    boutique,
                    detail.items,
                    detail.items.reduce((s, i) => s + i.prix * i.qty, 0),
                    detail.received,
                    detail.discount,
                    detail.pay_mode,
                    detail.receipt_num,
...
function Stats({ ventes, produits }) {
  ...
  const top = Object.entries(
    ventes.reduce((acc, v) => {
      v.items.forEach((it) => {
        if (!acc[it.id]) acc[it.id] = { nom: it.nom, qty: 0, total: 0 };
        acc[it.id].qty += it.qty;
        acc[it.id].total += it.prix * it.qty;
      });
      return acc;
    }, {})
  )
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <div>
          ...
          {Object.entries(byCat).length === 0 ? (
            <div style={{ color: '#ccc', fontSize: 13 }}>Pas de données</div>
          ) : (
                ...
                >
                  <div
                    style={{
                      ...
                      borderRadius: 3,
                      width: `${
                        (v / Math.max(...Object.values(byCat))) * 100
                      }%`,
                    }}
                  />
                </div>
          ...
          {top.length === 0 ? (
            ...
            top.map(([id, p], i) => (
              ...
              >
                <span>
                  <span
                    style={{ color: G.brand, fontWeight: 700, marginRight: 6 }}
                  >
                    #{i + 1}
                  </span>
                  {p.nom}
  ...
  return (
    <div style={{ maxWidth: 420 }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
        ...
        <Btn color="red" onClick={onLogout}>
          Se déconnecter
        </Btn>
      </div>
      <div
        style={{
          marginTop: 24,
          padding: '14px 16px',
          background: G.brandLight,
          borderRadius: 10,
        }}
      >
        <div style={{ fontWeight: 600, color: G.brandDark, fontSize: 13 }}>
          Plan : Gratuit (30 jours d'essai)
        </div>
        <div style={{ fontSize: 12, color: G.brandDark, marginTop: 4 }}>
      ...
      >
        ...
        {sideOpen && (
          ...
          >
            ...
            <div style={{ fontSize: 12, fontWeight: 600, color: G.brand }}>
              {boutique.nom}
            </div>
          </div>
        )}
          ...
          {NAVS.map((n) => (
            <div
              ...
              style={{
                ...
                cursor: 'pointer',
                fontSize: 13,
                color: view === n.id ? G.brand : '#555',
                fontWeight: view === n.id ? 600 : 400,
                background: view === n.id ? G.brandLight : 'transparent',
                borderRight:
                  view === n.id
                    ? `3px solid ${G.brand}`
        ...
        <div style={{ padding: '12px 14px', borderTop: '1px solid #f0f0f0' }}>
          <div
            ...
            style={{
              cursor: 'pointer',
              fontSize: 16,
              textAlign: sideOpen ? 'right' : 'center',
              color: '#bbb',
            }}
          >
            {sideOpen ? '◀' : '▶'}
      ...
      >
        ...
        >
          <div style={{ fontWeight: 600, fontSize: 15 }}>
            {NAVS.find((n) => n.id === view)?.icon}{' '}
            {NAVS.find((n) => n.id === view)?.label}
          </div>
          <div style={{ fontSize: 12, color: G.brand, fontWeight: 500 }}>
            ● En ligne
          </div>
        </div>
